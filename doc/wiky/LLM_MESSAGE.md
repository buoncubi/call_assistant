...
class MyLlmMessage private constructor(private val logger: CentralizedLogger) : LlmMessage<LlmMessageProvider> {
   ...
   companion object {
      ...
      fun build(logger: CentralizedLogger): LlmMessage.Builder<Message> = MyLlmMessage(logger).Builder()
   }
}

// As an example, get a generic logger in a dirty way. It usually should come from `MessagesManager.logger`.
object DummyLogger: Loggable(){
    val publicLogger = logger
}
val logger = DummyLogger.publicLogger

// Create a new instance.
val message : MyLlmMessage.Builder  = MyLlmMessage.build(logger)
...
class MyMetaMessage private constructor(
        private val llmMessage: LlmMessage<LlmMessageProvider>,
        private val logger: CentralizedLogger
    ) : MessageWrapper<LlmMessageProvider> {
   ...
   companion object {
      ...
      // Implementation of the factory paradigm.
      fun <M> build(llmMessageBuilder: LlmMessage.Builder<LlmMessageProvider>, 
                    logger: CentralizedLogger): MessageWrapper.Builder<LlmMessageProvider> =
         MyMetaMessage(llmMessageBuilder.public(), logger).Builder()
   }
}
...
// Create a new instance.
val messageWrapper : MyMetaMessage.Builder  = MyMetaMessage.build(MyLlmMessage.build(), logger)
...
```kotlin
val manager: MessageManager = MessagesManager { logger -> MyMetaMessage.build(MyLlmMessage.build(logger), logger) }
```
This allows `MessagesManager` to create instances of `MyMetaMessage` (which encodes instances of `MyLlmMessage`) at run
time. It also propagates the logger of `MessagesManager` to `MyMetaMessage` and `MyLlmMessage`
