...
data class PlaybackData(val event: PlaybackEvent?, override val sourceTag: String): CallbackInput {
    override fun copy() = PlaybackData(
        this.event,
        this.sourceTag
    )
}
...
object DesktopAudioPlayer : Text2SpeechPlayer<PlaybackData>(){
    ...
    override val onBeginPlayingCallbacks = CallbackManager<PlaybackData>(logger)
    override val onEndPlayingCallbacks = CallbackManager<PlaybackData>(logger)
    ...
    override suspend fun doComputeAsync(input: InputStream, sourceTag: String, scope: CoroutineScope) {
        ...
        player!!.playBackListener = object : PlaybackListener() {
            override fun playbackStarted(event: PlaybackEvent?) {
                ...
                onBeginPlayingCallbacks.invoke(PlaybackData(event, sourceTag), scope)
            }

            override fun playbackFinished(event: PlaybackEvent?) {
                logDebug("AWS Text-to-Speech playback started.")
                onEndPlayingCallbacks.invoke(PlaybackData(event, sourceTag), scope)
            }
        }
