 ...
 * playing. Such a callbacks takes as input a [C] object and return nothing. Note that [C] must be of type
 * [CallbackInput], which encodes a `sourceTag`. The latter is an identifier that you can se while using the
 * [ServiceInterface] functionalities, and it will be propagated to the callback. In the current implementation. the
 * `sourceTag` is propagated from the relative service's functionalities of [Text2Speech].
 *
 ...
 */
interface Text2SpeechPlayerInterface<C: CallbackInput>: ServiceInterface<InputStream> {
    // See documentation above.
    val onBeginPlayingCallbacks: CallbackManager<C>
    val onEndPlayingCallbacks: CallbackManager<C>
}
 ...
 *
 * Note that [C] must be of type [CallbackInput], which encodes a `sourceTag`. The latter is an identifier that you can
 * se while using the [ServiceInterface] functionalities, and it will be propagated to the callback. Note that the
 * `sourceTag` is propagated from the relative service's functionalities of [Text2Speech].
 *
 * @param C The type of the callbacks input.
 *
 ...
 * @author Luca Buoncompagni, © 2025, v1.0.
 */
abstract class Text2SpeechPlayer<C: CallbackInput>: Service<InputStream>(text2SpeechScope), Text2SpeechPlayerInterface<C>
 ...
 * for more.
 *
 * Finally, the errors involved on [Text2SpeechPlayer.onErrorCallbacks] are propagate to the callbacks added to
 * [onErrorCallbacks].
 *
...
abstract class Text2Speech(private val player: Text2SpeechPlayer<*>) : ReusableService<String>(text2SpeechScope) {
    ...
    /**
     * It is a reference to the callbacks that are invoked when the audio ends playing, i.e.,
     * [Text2SpeechPlayer.onEndPlayingCallbacks]. However, the callback added here will receive `Any` as input
     * parameter, while the callback added through [player] will revive that actual data type.
     */
    val onResultCallbacks = player.onEndPlayingCallbacks
     ...
     */
    protected abstract suspend fun fetchAudio(text: String): InputStream?
    ...
    override suspend fun doComputeAsync(input: String, sourceTag: String, scope: CoroutineScope) {
        ...
        // Fetch audio
        logger.trace("Service is fetching audio...")
        ...
        logger.debug("Service fetched audio (took {}).", fetchingTime)

        // Play the audio and wait for it to finish
        if (inputStream != null) {
            resetTimeout()
            logger.trace("Service is playing audio...")
            val playingTime = measureTime {
                player.computeAsync(inputStream, sourceTag = sourceTag) // Eventually, you can set a timeout here.
                player.wait(sourceTag = sourceTag)
            }
            logger.debug("Service played audio (took {}).", playingTime)
        } else
            logWarn("Service fetched a `null` audio.")
     ...
     *
     * @param sourceTag An identifier that is not processed by this class, but it is propagated to the on
     * error, and on timeout callbacks invoked by this function, which encompass also [player] functionalities.
     */
    override suspend fun doWait(sourceTag: String) {
        // player.wait(sourceTag = sourceTag) // It is already waiting for it in `doComputeAsync` above.
        super.doWait(sourceTag)
    }
     ...
     *
     * @param sourceTag An identifier that is not processed by this class, but it is propagated to the on error
     * callbacks that might be invoked by this function, which encompass also [player] functionalities.
     */
    override fun doStop(sourceTag: String) {
        player.stop(sourceTag)
