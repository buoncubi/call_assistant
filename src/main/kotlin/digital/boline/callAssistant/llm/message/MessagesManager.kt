...
class MessagesManager<M>(private val messageBuilder: (logger: CentralizedLogger) -> MessageWrapper.Builder<M>) : Loggable() {
    ...
    /* It is update when messages are added, and it is -1, when `mutableMessages` is empty; 0, where no summarization
       ...
       consecutive summarization) it might point out of list bound, thus, be mindfully of index range.
     */
    ...
    private fun addFistMessage(role: MetaRole, contents: List<String>): MessageWrapper.Builder<M> {
        if (role == MetaRole.ASSISTANT) {
            logError("The first message cannot be of type 'ASSISTANT'.")
    ...
    fun getSummaryInfo(): Summarizing {
        ...
        logInfo("Providing summary info: '{}'.", summarizing)
        return summarizing
    }
...
data class Summarizing(val messages: List<SummarizingMessage>,
     ...
     * ${Formatter.prefix}${Formatter.SUMMARY_TAG}${Formatter.suffix} $content1. ... $contentN.${Formatter.SUMMARY_CLOSURE}
     ...
     * If `false`[previousSummary] will always be ignored.
     * @return The formatted message to be summarized.
     */
    fun format(preamble: String = "", includePreviousSummary: Boolean = true): String {
        return StringBuilder().apply {
            // Add preamble
            ...
            if (messages.isNotEmpty())
                append(messages.joinToString(separator = "") { it.format() })
        }.toString()
    }
    ...
    data class SummarizingMessage(val role: MetaRole, val contents: List<String>, val index: Int, val id: String) {
         ...
         * {content2}, ... {contentN}{formatted.closure}"` (e.g., `**User**: My first message content. Another
         * content.\n` or `**Previous Summary**: A previous summary.\n\n`).
         *
        ...
        fun format(): String {
            return StringBuilder().apply {
                ...
                // Format contents
                val closure = if(role == MetaRole.SUMMARY) Formatter.summaryClosure else Formatter.closure
                append(contents.filter { it.isNotEmpty() }
                    .joinToString(separator = Formatter.separator, postfix = closure)
    ...
    object Formatter {
         ...
         * the input does not already end with a dot.
         */
        private val TRANSFORMER: (String) -> String = { it -> if (it.endsWith(".")) it else "$it." }
