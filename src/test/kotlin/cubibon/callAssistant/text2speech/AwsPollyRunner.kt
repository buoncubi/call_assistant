...
object AwsPollyRunner: Loggable() {
    ...
    fun runTest() = runBlocking{
        ...
        polly.activate()
        polly.computeAsync("Ciao", sourceTag = "TestSourceTag")
        polly.wait(Timeout(10_000) {id -> logWarn("Waiting timeout reached ($id)") })
        logInfo("-------------")
        polly.computeAsync("mondo")
        Thread.sleep(200)
        polly.stop()
        polly.deactivate()
        ...
        polly.activate()
        polly.computeAsync("Ciao", longTimeout)
        polly.wait()
        logInfo("-------------")
        polly.computeAsync("Mondo", shortTimeout, "MySourceTag")
        polly.wait()
        polly.deactivate()
